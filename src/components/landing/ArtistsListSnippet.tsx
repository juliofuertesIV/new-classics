...
export default function ArtistsListSnippet({ artists } : Props) {
    ...
    return (
        ...
        {
            artists.map((artist) => (
                <article key={ artist.id } className="group flex justify-end bg-zinc-900 w-full max-w-lg h-32 items-center relative border-4 border-black cursor-pointer hover:border-fuchsia-400">
                    <Image 
                    ...
                    <div className="flex items-center w-fit h-fit p-2 mr-4 text-white absolute">
                        <h3 className='drop-shadow-md group-hover:bg-fuchsia-400 transition-colors'>{ artist.name }</h3>
                    </div>
                </article>
            ))
        }
