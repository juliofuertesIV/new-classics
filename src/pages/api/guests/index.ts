...
export default function routeHandler (req: NextApiRequest, res: NextApiResponse) {
    ...
    switch (req.method) {
        case "GET": return getAllGuests(req, res)
...
const getAllGuests = async (req: NextApiRequest, res: NextApiResponse) => {

    const guests = await Guest.findAll({ order: [[ 'name', 'ASC' ]]}).then(data => data)
...
const addArtist = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
        const inserted = await Guest.create(payload)
